...
Vous pouvez stocker ces variables dans un fichier `database.properties` à la racine du projet dans le dossier config avec application.properties .
...
2. **Définissez** les variables d’environnement nécessaires (`DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USERNAME`, `DB_PASSWORD`) via votre terminal ou votre IDE.
   - *(Optionnel)* : créez un fichier `database.properties` pour stocker ces paramètres en local (**attention, Spring Boot ne charge pas automatiquement ce fichier, ajouter le chemin de variable d'environnement adequate**).
